            ...
            for (int i = 0; i <= str.Length; i++) {
                if (i == str.Length || str[i] == delimiter) {
                    if (i > start) {
                        ...
                        int trimStart = 0;
                        int trimEnd = sub.Length;
                        while (trimStart < trimEnd && sub[trimStart] == ' ') trimStart++;
                        while (trimEnd > trimStart && sub[trimEnd - 1] == ' ') trimEnd--;
                        result[idx++] = sub.Substring(trimStart, trimEnd - trimStart);
                        sub.Dispose();
                    } else {
                        result[idx++] = "";
                    }
